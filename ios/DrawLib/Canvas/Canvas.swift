...
open class Canvas: MetalView {
    ...
    var concurrentPhotoQueue:DispatchQueue?
    ...
    open func render(lines: [MLLine]) {
        data.append(lines: lines, with: currentBrush, drewIn: self.frame)
        // create a temporary line strip and draw it on canvas
        LineStrip(lines: lines, brush: currentBrush, drewIn: self.frame).drawSelf(on: screenTarget)
        /// submit commands
